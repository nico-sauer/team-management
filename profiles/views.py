...
def athlete_details(request, id=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("athlete-details", id=athlete.id)
        else:
            # keep the modal open and show errors
            logger.warning("athlete_details form invalid: %s", form.errors)
            logger.warning(
                "athlete_details request.FILES keys: %s", list(request.FILES.keys())
            )
            show_modal = True

    context = {"athlete": athlete, "form": form, "show_modal": show_modal}
    # if request.method == "POST":
...
def medical_records(request, id=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return redirect("medical-records", id=athlete.id)
        else:
            logger.warning("staff_details form invalid: %s", form.errors)
            logger.warning(
                "staff_details request.FILES keys: %s", list(request.FILES.keys())
            )
            show_modal = True
...
def dietary(request, id=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return redirect("medical-dietary", id=athlete.id)
        else:
            logger.warning("medical_records form invalid: %s", form.errors)
            logger.warning(
                "medical_records request.FILES keys: %s", list(request.FILES.keys())
            )
            show_modal = True
...
def staff_details(request, id=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return redirect("staff-details", id=staff_member.id)
        else:
            logger.warning("dietary form invalid: %s", form.errors)
            logger.warning("dietary request.FILES keys: %s", list(request.FILES.keys()))
            show_modal = True

    context = {"staff_member": staff_member, "form": form, "show_modal": show_modal}
    # if request.method == "POST":
